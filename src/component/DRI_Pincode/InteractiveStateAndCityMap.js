...
const InteractiveStateAndCityMap = () => {
  ...
  // Handle district click - simply open modal with district info
  const handleDistrictClick = (event, d) => {
    console.log("District clicked:", d);

    // Set the selected district info only
    setSelectedDistrict({
      district: d.properties.district,
      state: d.properties.st_nm,
    });

    // Open modal immediately - DistrictData will handle API calls
    setModalOpen(true);
  };
  ...
  return (
    ...
    >
      ...
      {/* Modal for District Data */}
      <Modal
        open={modalOpen}
        onClose={handleCloseModal}
        aria-labelledby="district-data-modal"
        aria-describedby="district-data-details"
        sx={{
          display: "flex",
          alignItems: "center",
