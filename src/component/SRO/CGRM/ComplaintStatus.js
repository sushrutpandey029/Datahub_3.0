...
const ComplaintStatus = ({ data }) => {
  console.log("ComplaintStatus data:", data);
  ...
  if (data && data.chart9 && Array.isArray(data.chart9)) {
    // API structure: data.chart9 contains array of {label, value, percentage}
    ...
    series = staticCategories.map(staticLabel => {
      ...
      const matchingItem = data.chart9.find(item => item && item.label === staticLabel);
  ...
  const pieChartOptions = {
    ...
    dataLabels: {
      ...
      formatter: function(val, opts) {
        ...
        const value = data && data.chart9 && data.chart9[opts.seriesIndex] ? data.chart9[opts.seriesIndex].value : "0";
    ...
    tooltip: {
      ...
      y: {
        formatter: function(val, { seriesIndex, w }) {
          ...
          const matchingItem = data && data.chart9 && data.chart9.find(item => item && item.label === category);
