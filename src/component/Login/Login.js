...
import {
  authLogin,
  authForgotPassword,
  authResetLoginStatus,
} from "../Service/Login/AuthService";
...
export default function Login() {
  ...
  const handlClick = async (e) => {
    ...
    try {
      ...
      if (error.response) {
        if (
          error.response.data &&
          error.response.data.access_status === false &&
          error.response.data.status === 200
        ) {
          // alert(error.response.data.message);
          const confirmLogout = window.confirm(
            error.response.data.message +
            "\n\nDo you want to logout from other sessions?"
          );
          if (confirmLogout) {
            handleResetLoginStatus(error.response.data.data.email);
          } else {
            console.log("user canceled the actions.");
          }
        }
      }
    }
  ...
  const handleResetLoginStatus = async (data) => {
    try {
      const body = {
        email: data,
      };
      console.log("data", body);
      const resp = await authResetLoginStatus(body);
      console.log("resp in auth reset", resp);
      if (resp.data.status) {
        alert(resp.data.message);
      }
    } catch (err) {
      console.log("error in login status", err);
    } finally {
      setIsDisabled(false);
    }
  };
        ...
        <div className="form-content">
          <div className="form-items with-bg">
            ...
            <p className="copyright">
              Copyright &copy;{" "}
              <a href="https://mfinindia.org/">
                Microfinance Industry Network. 2024.
              </a>
            </p>
          </div>
        </div>
      ...
      >
        <Box sx={style}>
          ...
          <Box
            component="form"
            noValidate
            onSubmit={(e) => handleForgotClick(e)}
            sx={{ mt: 1 }}
          >
            <TextField
              margin="normal"
